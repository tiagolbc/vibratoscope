...
def extract_pitch_sfeeds(audio_data, sr, fmin=50.0, fmax=700.0, time_step=0.0033, file_path=None, **kwargs):
    import parselmouth
    import numpy as np
    ...
    times = f0_contour[:, 0]
    f0s = f0_contour[:, 1]
    # Save TXT if file_path is provided
    if file_path is not None:
        import pandas as pd
        out_txt = os.path.splitext(file_path)[0] + "_pitch_sfeeds.txt"
        df = pd.DataFrame({
            "time_s": times,
            "f0_Hz": f0s
        })
