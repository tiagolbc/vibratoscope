...
class VibratoGUI:
    def __init__(self, root):
        ...
        tk.Label(pitch_frame, text="Pitch Method:", bg="#b0afa6", font=cal_font).pack(side=tk.LEFT, padx=(5, 5))
    ...
    def extract_pitch(self, fmin, fmax):
        ...
        if self.pitch_method.get() == "praat":
            ...
            self.times, self.pitch_hz = extract_pitch_with_yin(
                self.audio_data, self.sr, fmin=fmin, fmax=fmax
            )
            print("Pitch extracted with YIN successfully.")
            messagebox.showinfo("Pitch Extraction", "Pitch extracted with YIN successfully.")
        elif self.pitch_method.get() == "reaper":  # <<<<<<<< ADICIONE ESSA PARTE
    ...
    def run_analysis(self):
        ...
        save_dir = filedialog.askdirectory(title="Select folder to save analysis results")
        if not save_dir:
    ...
    def batch_process(self):
        ...
        if not save_dir:
            messagebox.showinfo("Batch Processing", "No output directory selected.")
            return
